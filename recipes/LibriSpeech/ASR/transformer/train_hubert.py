...
import sys
import torch
torch.cuda.set_device(torch.device("cuda:" + str(0)))
import logging
...
class ASR(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        enc_out, src_key_padding_mask = self.modules.wav2vec2(wavs, wav_lens)
        
        # print(enc_out.shape, src_key_padding_mask.shape, tokens_bos.shape, wav_lens.shape, wavs.shape)
...
if __name__ == "__main__":
    ...
    # asr_brain = asr_brain.to('cuda:' + str(local_rank))
    # asr_brain = DDP(asr_brain, device_ids=[local_rank])
